        ...
        props.search(props.modelValue, (list: any) => {
            ...
            list.forEach((item: any) => {
                ...
                props.titles.forEach((title: string) => {
                    item.dropdownTitle += `<span class="item-found item-width-${props.titles.length}">${item[title]}</span>`
                })
...
<template>
    <div class="row g-2 align-items-center">
        <div v-if="props.label" class="col-2 d-flex justify-content-end">
            <label class="col-form-label form-control-sm me-0 p-1" :for="elementId">{{ props.label }}
                <i class="bi bi-search ps-2"></i>
            </label>
        </div>
        <div class="col">
            <div class="form-control border-0 p-0 dropdown-container ">
                <input type="text" v-model="value" class="form-control form-control-sm" :id="elementId"
                ...
                <ul v-if="items.length > 0" class="list-group scrollable-dropdown" ref="refDropdown"
                    ...
                    <li v-for="(item, index) in items" :key="index" tabindex="0" class="list-group-item form-control-sm"
